...
ReadFile<T>::ReadFile(const FilePathFormat& fileName)
...
template<BinarySerializable T>
template<validfstreamFilePathFormat FilePathFormat>
void ReadFile<T>::open(const FilePathFormat& fileName)
{
...
void ReadFile<T>::getColumn(Container& data, const::std::string& columnName) 
...
void ReadFile<T>::getColumn(Container& data, const std::string& columnName) noexcept
{
	assert(contains(columnName) && "the file does not contain a column of this name");
...
template<typename Container>
	requires DynamicArrayConcept<Container, T>
void ReadFile<T>::getColumn(Container& data, 
...
void ReadFile<T>::getColumns(Container& data, const StringContainer& columnNames) 
{
...
void ReadFile<T>::getColumns(Container& data, const StringContainer& columnNames) noexcept {
	using OuterContainerTraits = ContainerTraits<Container>;
	using InnerContainerTraits = ContainerTraits<range_value_t<Container>>;
	assert(std::ranges::all_of(columnNames, [&](const auto& name) {return contains(name); }) &&
		"one or more of the names does not exist in the file");
...
template<typename Container>
	requires DynamicArrayConcept<Container, T>
void ReadFile<T>::getRow(Container& data, size_t row) 
	noexcept(TraitReserveNoexceptV<Container>)
{
...
template<typename Container>
	requires ArrayConcept<Container, T>
void ReadFile<T>::getRow(Container& data, size_t row) noexcept {
	assert(row <= _rowCount && "The specified row is outside the buffer");
...
template<typename Container>
	requires DynamicArrayConcept<Container, T>
bool ReadFile<T>::getRow(Container& data, std::function<bool(const Container&)> condition)
{
	moveReader(0);
	ContainerTraits<Container>::reserve(data, columnCount());
	for (auto i = 0; i < _rowCount; i++) {
...
template<typename Container>
	requires NestedDynamicArray<Container, T>
void ReadFile<T>::getRows(Container& data, std::function<bool(const range_value_t<Container>&)> condition)
{
	using OuterContainerTraits = ContainerTraits<Container>;
	using InnerContainerTraits = ContainerTraits<range_value_t<Container>>;
...
template<typename Container>
	requires NestedDynamicArray<Container, T>
void ReadFile<T>::getAllColumns(Container& data) 
	noexcept(TraitReserveNoexceptV<Container> && TraitConstructCapacityNoexceptV<range_value_t<Container>>)
{
	using OuterContainerTraits = ContainerTraits<Container>;
	using InnerContainerTraits = ContainerTraits<range_value_t<Container>>;
...
template<typename Container>
	requires NestedArray<Container, T>
void ReadFile<T>::getAllColumns(Container& data) noexcept
{
	using OuterContainerTraits = ContainerTraits<Container>;
	using InnerContainerTraits = ContainerTraits<range_value_t<Container>>;
...
template<typename Container>
	requires NestedDynamicArray<Container, T>
void ReadFile<T>::getAllRows(Container& data)
	noexcept(TraitReserveNoexceptV<Container> && TraitConstructCapacityNoexceptV<range_value_t<Container>>)
{
	using OuterContainerTraits = ContainerTraits<Container>;
	using InnerContainerTraits = ContainerTraits<range_value_t<Container>>;
...
template<typename Container>
	requires NestedArray<Container, T>
void ReadFile<T>::getAllRows(Container& data) noexcept {
	using OuterContainerTraits = ContainerTraits<Container>;
	using InnerContainerTraits = ContainerTraits<range_value_t<Container>>;
...
template<BinarySerializable T>
const::std::vector<String>& Database::ReadFile<T>::getColumnNames() const noexcept
{
	return _columnNames;
}
template<BinarySerializable T>
const unsigned int ReadFile<T>::columnCount() const noexcept {
	return _columnNames.size();
}
template<BinarySerializable T>
const bool ReadFile<T>::contains(const CharSpan& name) const noexcept{
	return std::find(_columnNames.begin(), _columnNames.end(), name) != _columnNames.end();
}
template<BinarySerializable T>
const unsigned int Database::ReadFile<T>::rowLengthBytes() const noexcept
{
	return columnCount() * sizeof(T);
}
template<BinarySerializable T>
const size_t Database::ReadFile<T>::dataLengthBytes() const noexcept
{
	return _rowCount * rowLengthBytes();
}

template<BinarySerializable T>
const size_t ReadFile<T>::itemOffsetBytes(unsigned int column) const noexcept
{
	return column * sizeof(T);
}

template<BinarySerializable T>
const unsigned int ReadFile<T>::column(const CharSpan& name) const noexcept {
	return std::distance(_columnNames.begin(), std::find(_columnNames.begin(), _columnNames.end(), name));
}

template<BinarySerializable T>
void ReadFile<T>::moveReader(::std::streamoff offset, ::std::ios_base::seekdir dir) noexcept
{
...
template<typename Container>
requires DynamicArrayConcept<Container, T>
void ReadFile<T>::readRow(Container& data) noexcept
{
...
template<BinarySerializable T>
T ReadFile<T>::readItem() noexcept
{
	T x;
...
template<BinarySerializable T>
template<validfstreamFilePathFormat FilePathFormat>
void ReadFile<T>::setup(const FilePathFormat& filePath) {
